...
// Helper for type-safe API error messages
...
export function ProfilePage() {
  ...
  // Synchronize form fields when user state resolves or updates
  useEffect(() => {
    if (user) {
  ...
  // Fetch active devices when navigating to Security tab
  useEffect(() => {
  ...
  const handleRevokeOtherDevices = async () => {
    setIsSaving(true)
    const previousDevices = [...devices]
    // Optimistic removal of non-current devices
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Manage your personal details, active sessions, security preferences, or get support.
        </p>
      </div>
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 gap-6" role="tablist">
        <button
          ...
          onClick={() => { setActiveTab('profile'); resetAlerts(); }}
        ...
        <button
          ...
          onClick={() => { setActiveTab('security'); resetAlerts(); }}
          className={`pb-3 text-sm font-semibold transition-all relative ${
            activeTab === 'security'
              ? 'text-emerald-500 border-b-2 border-emerald-500'
              : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300'
        ...
        <button
          ...
          onClick={() => { setActiveTab('support'); resetAlerts(); }}
          className={`pb-3 text-sm font-semibold transition-all relative ${
            activeTab === 'support'
              ? 'text-emerald-500 border-b-2 border-emerald-500'
              : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300'
      ...
      {error && (
        <div className="text-sm font-medium text-red-600 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3">
          {error}
        </div>
      )}
              ...
              <div className="relative shrink-0 group">
                {avatarUrl ? (
                  <img
                    ...
                    className="h-20 w-20 rounded-full object-cover ring-4 ring-emerald-500/30 shadow-md"
                  />
                ) : (
                  <div className="h-20 w-20 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold text-xl flex items-center justify-center border-2 border-emerald-500/30">
                    {userInitials}
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 w-full space-y-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className="px-4 py-2 text-xs font-bold bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 rounded-xl transition-all"
                  >
                    Upload Photo
                  </button>
                  {avatarUrl && (
                    <button
                      ...
                      className="px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all"
                    >
                      Remove
                    </button>
                  )}
          ...
          <div className="card p-6 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" className="input" value={fullName} onChange={handleInputChange(setFullName)} />
              </div>
              <div>
                <label className="label">Email Address</label>
                <input type="email" disabled value={user.email} className="input bg-slate-100 dark:bg-slate-800 text-slate-500 cursor-not-allowed" />
              </div>
              <div>
                <label className="label">University</label>
                <input type="text" className="input" value={university} onChange={handleInputChange(setUniversity)} />
              </div>
              <div>
                <label className="label">Year of Study</label>
                <input type="number" min={1} max={6} className="input" value={yearOfStudy} onChange={handleInputChange(setYearOfStudy)} />
              </div>
              <div>
                <label className="label">Exit Exam Date</label>
                <input type="date" className="input" value={examDate} onChange={handleInputChange(setExamDate)} />
              </div>
              <div>
                <label className="label">Available Weekly Study Hours</label>
                <input type="number" min={0} className="input" value={weeklyHours} onChange={handleInputChange(setWeeklyHours)} />
              </div>
            </div>
          </div>

          <button type="submit" disabled={isSaving} className="w-full py-3 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-sm rounded-xl shadow-md transition-all disabled:opacity-50">
      ...
      {activeTab === 'security' && (
        <div className="space-y-6">
          ...
          <form onSubmit={handlePasswordSubmit} className="card p-6 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
            ...
            <button type="submit" disabled={isSaving} className="py-2.5 px-5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-sm rounded-xl transition-all disabled:opacity-50">
            ...
            {is2FAEnabled && (
              <div className="space-y-3">
                ...
                <input
                  ...
                  value={twoFACode}
                  onChange={(e) => setTwoFACode(e.target.value)}
                  className="input text-center text-2xl tracking-widest font-mono max-w-[150px]"
                />
                <button
                  onClick={handle2FADisable}
                  disabled={twoFACode.length !== 6}
                  className="py-2 px-4 bg-rose-500 text-white font-bold text-xs rounded-xl hover:bg-rose-600 transition-all disabled:opacity-50"
                >
                  Disable 2FA
                </button>
              </div>
          ...
          <div className="card p-6 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              {/* Display Sign Out button only when non-current sessions exist */}
              {hasOtherDevices && (
                <button
                  type="button"
                  onClick={handleRevokeOtherDevices}
                  disabled={isSaving}
                  className="px-3 py-1.5 text-xs font-semibold text-amber-700 dark:text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 rounded-xl transition-all disabled:opacity-50"
                >
            ...
            <div className="space-y-3 pt-2">
              {isLoadingDevices ? (
                <p className="text-xs text-slate-500 py-4 text-center">Loading active devices...</p>
              ) : devices.length === 0 ? (
                <p className="text-xs text-slate-500 py-4 text-center">No active devices found.</p>
              ) : (
                devices.map((device) => (
                  <div
                    key={device.id}
                    className="flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 mt-0.5">
                          {device.browser} • {device.ip_address} • Last active: {formatLastActive(device.last_active)}
                        </p>
                      </div>
                    </div>

                    {/* Show badge for current session, Revoke button for others */}
                    {device.is_current_device ? (
                      ...
                      <button
                        ...
                        className="text-xs font-medium text-red-600 hover:text-red-700 hover:bg-red-500/10 px-3 py-1.5 rounded-lg transition-all disabled:opacity-50"
                      >
                        {revokingId === device.id ? 'Signing out...' : 'Revoke'}
                      </button>
                    )}
                  </div>
                ))
          ...
          <div className="card p-6 border border-red-500/20 bg-red-500/5 rounded-2xl space-y-3">
            ...
            <p className="text-xs text-slate-500">
              Need to take a break or reset your progress? Contact support or initiate account deletion.
            </p>
            <button
              type="button"
              onClick={async () => {
                if (confirm('Are you sure you want to deactivate your account? You can request reactivation by contacting support.')) {
                  try {
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Issue with Exit Exam Practice Quiz"
                  className="input"
                  value={supportSubject}
                  onChange={handleInputChange(setSupportSubject)}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleInputChange(setSupportMessage)}
                />
              </div>
            </div>

            <button type="submit" disabled={isSaving} className="py-2.5 px-5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-sm rounded-xl transition-all disabled:opacity-50">
              {isSaving ? 'Sending…' : 'Send Ticket'}
            </button>
          </form>
          ...
          <div className="card p-6 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3">
            ...
            <div className="space-y-2 text-xs text-slate-500">
              <details className="cursor-pointer border-b border-slate-200 dark:border-slate-800 pb-2">
                <summary className="font-semibold text-slate-700 dark:text-slate-300">How does the AI Tutor learn my focus areas?</summary>
                <p className="mt-1 pl-2">The AI automatically tracks your quiz scores across different Computer Science topics and updates your strengths/weaknesses summary.</p>
              </details>
              <details className="cursor-pointer border-b border-slate-200 dark:border-slate-800 pb-2">
                <summary className="font-semibold text-slate-700 dark:text-slate-300">Can I reset my study progress?</summary>
                <p className="mt-1 pl-2">Yes, you can submit a request through the support form above to clear your quiz history.</p>
              </details>
            </div>
          </div>
        </div>
